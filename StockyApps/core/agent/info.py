...
PIPELINE_INFO = {
    ...
    "phase6_execute": {
        ...
        "text": (
            ...
            "B. BUYS SECOND — process buy signals with available capital.\n"
            "   Position size: 20% of current BP or 1/5 of cycle-start BP.\n"
            "   Tries bracket order (with stop-loss + take-profit) first.\n"
            "   Falls back to simple market order if bracket is rejected.\n\n"
            "C. CAPITAL ROTATION — if BP is too low for a high-confidence BUY,\n"
            "   the agent can sell the weakest position to fund it.\n"
    ...
    "risk_management": {
        ...
        "text": (
            ...
            "- Uses day-trading buying power to prevent 403 rejections\n"
            "- Bracket orders with stop-loss and take-profit targets\n"
            "- Concentration cap: max 10% of portfolio in one stock\n"
            "- Diversity multiplier reduces size when too many positions open\n"
            "- Tradability check before every buy (cached across cycles)"
        ),
    },
}
